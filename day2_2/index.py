...
def getRGBFromRound(round):
    if 'red' not in round.keys():
        round['red'] = 0
    if 'green' not in round.keys():
        round['green'] = 0
    if 'blue' not in round.keys():
        round['blue'] = 0
    return int(round['red']), int(round['green']), int(round['blue'])

def getPowerOfGame(r, g, b):
    return r*g*b
...
def getSumPower(matches):
    currentPower = 0
    for i in range(len(matches)):
        minR, minG, minB = 0,0,0
        matchId, rounds = formatGame(matches[i])
        for k in range(len(rounds)):
            r, g, b = getRGBFromRound(rounds[k])
            if r > minR:
                minR = r
            if g > minG:
                minG = g
            if b > minB:
                minB = b
        currentPower += getPowerOfGame(minR, minG, minB)
    return currentPower

print(getSumPower(arrFromText))
